...
IMAGE_FILES = {
    "standby": get_resource_path("standby.gif"),
    "click": get_resource_path("clickRock.gif"),
    "computer": get_resource_path("./assets/computer.gif"),
    "standbyFrog": get_resource_path("./assets/standbyFrog.gif"),  # <--- 新加
    "up": get_resource_path("up.gif"),
    "down": get_resource_path("down.gif"),
    "left": get_resource_path("left.gif"),
    "right": get_resource_path("right.gif"),
    "frog_run": get_resource_path("./assets/frog/frogRun.gif"),
    "frog_catch": get_resource_path("./assets/frog/frogAnimation22.gif"),
    "frogMini": get_resource_path("./assets/frog/frogMini.gif"), # <--- 小青蛙
}

EMOJI_PATHS = {
    "wenhao": get_resource_path("./assets/biaoq/EmojiWenhao.jpg"),
    "wuyu": get_resource_path("./assets/biaoq/EmojiWuyu.jpg"),
    "fannao": get_resource_path("./assets/biaoq/EmojiFannao.jpg"),
    "liuhan": get_resource_path("./assets/biaoq/EmojiLiuHan.jpg"),
    "smile": get_resource_path("./assets/biaoq/EmojiSmile.jpg"),
    "love": get_resource_path("./assets/biaoq/EmojiLove.jpg"),
...
def load_gif_raw(path, gif_name=None):
    ...
    try:
        while True:
            ...
            if gif_name == "standbyFrog":

                extra_frog_count = max(0, TOTAL_FROG_COUNT - 1)
                frog_config = ANIM_CONFIG["frogMini"]

                frog_anchor_x = frog_config["anchor_x"]
                frog_anchor_y = frog_config["anchor_y"]

                # 主角色脚底
                ground_y = canvas_size[1]

                for i in range(extra_frog_count):

                    frog_frame = frog_mini_frames[
                        img.tell() % len(frog_mini_frames)
                    ]
                    ...
                    # 脚底对齐
                    frog_y = ground_y - frog_anchor_y

                    transparent_bg.paste(
                        frog_frame,
                        (frog_x,frog_y),
                        frog_frame
                    )
            ...
            # 限制延迟在正常范围，避免过快或过慢
            validated_delay = max(MIN_FRAME_DELAY, min(frame_delay, MAX_FRAME_DELAY))
            delays.append(validated_delay)
...
def preload_all_gif():
    ...
    global frog_mini_frames
    global frog_mini_delays

    # ====================
    # 先加载小青蛙
    # ====================

    mini_frames, mini_delays = load_gif_raw(
        IMAGE_FILES["frogMini"],
        "frogMini"
    )
    ...
    frog_mini_frames = mini_frames
    frog_mini_delays = mini_delays
...
class SebPet(tk.Tk):
    def __init__(self):
        ...
        self.walk_event_cooldown = 1000*60 * 0.5
    ...
    def load_emoji_images(self):
        ...
        for key, path in EMOJI_PATHS.items():
            im = Image.open(path).resize(size, Image.Resampling.NEAREST)
    ...
    def on_press(self, e):
        ...
        # 待机状态（in_computer）下，不触发拖拽初始化和动画切换，仅保留点击计数
        if self.in_computer:
            self.click_count += 1
            if self.click_timer:
                self.after_cancel(self.click_timer)
    ...
    def show_emoji(self, emoji_key):
        ...
        self.emoji_win = wnd
        lbl = tk.Label(wnd, bg=self.TRANS, image=emoji_images[emoji_key])
        lbl.pack()
    ...
    def switch_anim(self):
        ...
        elif self.talking or self.menu_open or self.mouse_hover:
            # 鼠标悬浮时 永远只显示 standby 不会乱切
            self.set_anim("standby")
        ...
        if not self.menu_open and not self.talking and not self.dragging:
            if self.walk_active and not self.mouse_hover:
                ...
                if now - self.last_walk_event_check >= self.walk_event_cooldown:
                    ...
                    frog_chance = max(0.7, 0.8 - self.total_frog_caught * 0.05)
